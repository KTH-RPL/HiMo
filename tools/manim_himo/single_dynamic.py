...
class EgoMotion(Scene):
    ...
    def construct(self):
        ...
        for inter_ in range(0, 360, inter_set):
            ...
            for obj in [obj1, obj2, obj3]:
                ray, dot, angle = self.lidar_ray(ego_pos, obj.get_vertices(), start=inter_-90, end=inter_+inter_set-90)
                rays.extend(ray)
                dots.extend(dot)
                angles.extend(angle)
            ...
            self.play(Gego.animate.shift(UP*inter_*0.2/360), \
                    obj1.animate.shift(DOWN*2*inter_set/360), \
                    obj2.animate.shift(UP*inter_set/360), \
                    obj3.animate.shift(UP*2*inter_set/360), \
                      runtime=0.001, rate_func=linear)
            # self.wait(0.1)
